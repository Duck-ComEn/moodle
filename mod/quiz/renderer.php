...
class mod_quiz_renderer extends plugin_renderer_base {
    ...
    public function view_table($quiz, $context, $viewobj) {
        ...
        if ($viewobj->gradecolumn) {
            $table->head[] = 'Percent' ;
            //. ' / ' .
           //         quiz_format_grade($quiz, $quiz->grade/quiz_format_grade($quiz, $quiz->sumgrades)*100);
            $table->align[] = 'center';
            $table->size[] = '';
        }
    ...
    public function view_result_info($quiz, $context, $cm, $viewobj) {
        ...
        if ($viewobj->overallstats) {
            if ($viewobj->moreattempts) {
             ...
             //PASS OR FAIL
                if($viewobj->mygrade < $quiz->grade)
                    $resultinfo .= '<p class="FeedbackFail">'."YOU FAIL"."</p>\n";
                else
                    $resultinfo .= '<p class="FeedbackPass">'."YOU PASS"."</p>\n";
        }
