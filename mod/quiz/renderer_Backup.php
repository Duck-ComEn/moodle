	   ...
	   $table->head[] = get_string('feedback', 'quiz');
        ...
        foreach ($viewobj->attemptobjs as $attemptobj) {
			...
			if ($viewobj->gradecolumn) {
                if ($attemptoptions->marks >= question_display_options::MARK_AND_MAX &&
                        $attemptobj->is_finished()) {
					$row[] = number_format((((quiz_format_grade($quiz, $attemptgrade)+0)/(quiz_format_grade($quiz, $quiz->grade)+0))*100),2,'.','' )." %";
                } else {
                    $row[] = '';
                }
            }
			...
			*/
			...
			//Add FeedBack Row
                if ((quiz_format_grade($quiz, $attemptgrade)+0)==(quiz_format_grade($quiz, $quiz->grade)+0)) {
                    $row[] = "PASS";
                } else {
                    $row[] = 'FAIL';
                }
