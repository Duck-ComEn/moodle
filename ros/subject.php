...
<head>
	...
	<?php
		...
		case "admin" : 
	?>
	<title>Benchmark Garde - Administrator::รายวิชา (Manual)</title>
			...
			<div class="body_textarea">
				<div align="justify">
				  ...
				  echo"<table id='mytable' cellspacing='0'>";
						...
						while($data = mysql_fetch_array($result)){
						echo "<tr><td>$i</td><td><form action='update-code-menu.php'><input name=\"id\" type=\"hidden\" value=\"{$data['id']}\"><input type=\"text\" name=code value=\"{$data['code']}\"><input type=\"submit\" value=\"แก้ไข\"></form></td><td><form action='update-subname-menu.php'><input name=\"id\" type=\"hidden\" value=\"{$data['id']}\"><input name=\"name1\" type=\"hidden\" value=\"{$data['name']}\"><input type=\"text\" name='name' value=\"{$data['name']}\"><input type=\"submit\" value=\"แก้ไข\"></form></td><td align=center>";if($data['vision']==1){echo"<a href=\"del-vision.php?id={$data['id']}\"><img src=\"images/eye1.png\"/></a>";}else{echo"<a href=\"del0-vision.php?id={$data['id']}\"><img src=\"images/eye0.png\"/></a>";} echo"</td><td align=center><a href=\"del-subject.php?id={$data['id']}\">Delete</a></td></tr>";
						$i++;
						}
						echo"</table>";
				  echo"</table>";
